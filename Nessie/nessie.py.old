...
def main():
    ...
    if "node_logs" in data:
        logger.info(f"  - Node logs: {len(data['node_logs'])} services")
    if "k8s_configs" in data and "namespaces" in data["k8s_configs"]:
        logger.info(f"  - Kubernetes: {len(data['k8s_configs']['namespaces'])} namespaces")
    if "pod_logs" in data:
        pod_count = len([k for k in data['pod_logs'].keys() if k != "error"])
        logger.info(f"  - Pod logs: {pod_count} pods")
    if "node_metrics" in data and "items" in data["node_metrics"]:
        logger.info(f"  - Node metrics: {len(data['node_metrics']['items'])} nodes")
    if "versions" in data:
        logger.info(f"  - Versions: {len(data['versions'])} components")
